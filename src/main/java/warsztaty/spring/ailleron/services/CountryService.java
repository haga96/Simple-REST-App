...
public class CountryService {
    ...
    public RestResponse getCountryFromRestApi(String code){
        ...
        if(response.getStatusCode().is2xxSuccessful()){
            response.getBody().getRestResponse().setHello(messageSource.getMessage("hello.message",null, Locale.forLanguageTag(code)));
        }
        return response.getBody().getRestResponse();
    }

    private ResponseEntity<RootObject> getDataFromApi(String code) {
        return restTemplate.getForEntity(COUNTRY_CODE_URL + code, RootObject.class);
    }
}
